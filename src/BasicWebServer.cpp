...
void WebServer::handleClient() {
  if (_currentStatus == HC_NONE) {
    MY_CLIENT client = _server.available();
    if (!client) {
      return;
    }
...
void WebServer::send_P(int code, PGM_P content_type, PGM_P content, size_t contentLength) {
    ...
    memccpy_P((void*)type, (PGM_VOID_P)content_type, 0, sizeof(type));
    _prepareHeader(header, code, (const char* )type, contentLength);
    sendContent(header.c_str(), header.length());
...
void WebServer::send(int code, const String& content_type, const char* content, size_t len) {
	String header;
	_prepareHeader(header, code, content_type.c_str(), len);
	_currentClientWrite(header.c_str(), header.length());
	if (len) sendContent(content, len);
}
...
void WebServer::sendJsonDoc(int code, const DynamicJsonDocument  doc)
{
	//void WebServer::send(int code, const String& content_type, const char* content, size_t len) {
		String header;
		size_t len = measureJson(doc);
		_prepareHeader(header, code, "application/json", len);
		_currentClientWrite(header.c_str(), header.length());
		if (len) 
		{
			const char * footer = "\r\n";
			...
			// bufferd, sonst wird jedes Byte einzeln gesendet
			WriteBufferingStream bufferdClient(_currentClient, 1024);
...
//Workaround for writing streams via ethernet
size_t WebServer::_writeStream(Stream &stream)
{
	...
	while (available) {
		...
		written += _currentClient.write(buf, toWrite);
